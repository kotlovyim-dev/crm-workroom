...
from app.features.shared.models import CalendarEvent, Employee, EmployeeProfile, TimeOffRequest, VacationBalance
...
class DomainSupport:
    ...
    async def _get_or_create_balance(self, session: AsyncSession, employee_id: str, year: int) -> VacationBalance:
    ...
    async def _get_workspace_event(self, session: AsyncSession, workspace_id: str, event_id: str) -> CalendarEvent:
    ...
    def _build_user_profile(self, employee: Employee, profile: EmployeeProfile | None) -> UserProfile:
        return UserProfile(
            id=employee.id,
            workspace_id=employee.workspace_id,
            first_name=employee.first_name,
